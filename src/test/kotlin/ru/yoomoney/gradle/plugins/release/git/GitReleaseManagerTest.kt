...
import ru.yoomoney.gradle.plugins.release.AbstractReleaseTest
import java.io.File
...
class GitReleaseManagerTest : AbstractReleaseTest() {
    ...
    @Before
    fun before() {
        gitReleaseManager = GitManager(File(projectDir.root.absolutePath), GitSettings("", "", null, null))
    ...
    fun `should push to repo with ssh `() {
        val gitSettings = GitSettings(
                email = "",
                username = "",
                sshKeyPath = userKey.path,
                passphraseSshKey = null
        )

        val gitManager = GitManager(File(projectDir.root.absolutePath), gitSettings)
